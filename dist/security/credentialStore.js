...
export function detectCredentialEnvironment(env = process.env, platform = process.platform, release = os.release()) {
...
function getCredentialStoreMode() {
...
function isWslRuntime(env = process.env, platform = process.platform, release = os.release()) {
    if (platform !== "linux")
        return false;
