...
- Client integration UX (2026-03-27): completed first pass with a dedicated control-plane `Clients` tab that can auto-add `mcp-for-i` to locally installed Codex, Claude Code, and VS Code/GitHub Copilot clients, plus manual snippets for Project Bob and generic MCP clients when no stable local automation path exists.
- Phase 8-10: pending
